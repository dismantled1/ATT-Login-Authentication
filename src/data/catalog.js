...
const mergeRecord = (base, patch) => ({
  ...
  projectIds: Array.isArray(patch.projectIds) && patch.projectIds.length > 0 ? patch.projectIds : base.projectIds,
  memberUids: Array.isArray(patch.memberUids) && patch.memberUids.length > 0 ? patch.memberUids : base.memberUids,
  ...
  milestones: Array.isArray(patch.milestones) && patch.milestones.length > 0 ? patch.milestones : base.milestones,
  touchpoints: Array.isArray(patch.touchpoints) && patch.touchpoints.length > 0 ? patch.touchpoints : base.touchpoints
});
...
const preserveCustomEntries = (stored, defaults, getKey) => {
  ...
  return stored.filter((item) => { const key = getKey(item); return key && !defaultKeys.has(key); });
};
...
export const withResolvedRole = (user) => (user ? { ...user, role: resolveRole(user) } : null);
...
export const getScopedProjects = (user, projects) => {
  ...
  if (role === 'tenant_admin') {
    return projects.filter((project) => project.tenantAdminUid === user.uid || project.tenantId === user.tenantId);
  }
  return projects.filter((project) => user.projectIds?.includes(project.id) || project.memberUids?.includes(user.uid));
};
...
export const getScopedUsers = (user, users, scopedProjects) => {
  ...
  const scopedMemberIds = new Set(scopedProjects.flatMap((project) => [
    project.tenantAdminUid, project.transitionManagerUid, ...(project.memberUids || [])
  ]));
